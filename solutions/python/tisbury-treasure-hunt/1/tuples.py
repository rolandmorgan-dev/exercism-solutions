...
def clean_up(combined_record_group):
    ...
    for item in combined_record_group:
        concatenated_value  = ''.join(item[3])
        if item[1] == concatenated_value:
            cleaned_records.append(f"('{item[0]}', '{item[2]}', {item[3]}, '{item[4]}')")
        else:
            cleaned_records.append(f"('{item[0]}', '{item[1]}', '{item[2]}', {item[3]}, '{item[4]}')")
