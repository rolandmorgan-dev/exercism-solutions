...
def resistor_label(colors):
    if colors[0] == "black": return "0 ohms"
    if len(colors) == 4:
    ...
    for key, value in resistances.items():
        if result // value >= 1:
            if (result/value).is_integer(): return f"{result // value} {key} {tolerances[tolerance]}"
            else: return f"{result / value} {key} {tolerances[tolerance]}"
